describe('score_to_grade_spec', function() {
  ...
  it('input String return invalid input', function() {
    expect(function(){ score_to_grade(data6); }).toThrow(new Error("is not a number"));
  });
  ...
  it('input is String type number and input < 60  and input >= 0 return  D', function() {
    var result = score_to_grade(data8);
    expect(result).toEqual("D");
  });

  it('input is null return invalid input', function() {
    expect(function(){ score_to_grade(data9); }).toThrow(new Error("is not a number"));
  });
